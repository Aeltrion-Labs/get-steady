        ...
        <div className="space-y-4">
          ...
          <div className="space-y-3">
            ...
            <p className="max-w-2xl text-sm leading-6 text-muted-foreground">
              Stay close to the day: log what came in, what went out, and close the loop before
              tomorrow.
            </p>
          </div>
          <div className="grid gap-3 sm:grid-cols-2 xl:max-w-[34rem]">
            <div className="rounded-[24px] border border-border/80 bg-muted/50 p-4">
              ...
              <p className="mt-2 text-lg font-semibold text-foreground">
                {summary.isTodayCheckedIn
                  ? "You are closed out for today."
                  : "Today is still open."}
              </p>
              <p className="mt-1 text-sm text-muted-foreground">
                {summary.isTodayCheckedIn
                  ? "Use quick add only if something changed."
                  : "A short pass now keeps tomorrow lighter."}
              </p>
            </div>
            <div className="rounded-[24px] border border-border/80 bg-accent/55 p-4">
              ...
              <p className="mt-2 text-lg font-semibold text-foreground">
                {summary.missedCheckInDaysCount === 0
                  ? "No recovery lane waiting."
                  : `${summary.missedCheckInDaysCount} missed day${summary.missedCheckInDaysCount === 1 ? "" : "s"}`}
              </p>
              <p className="mt-1 text-sm text-muted-foreground">
        ...
        <div className="flex flex-col gap-4 rounded-[28px] border border-border/80 bg-slate-950 px-5 py-5 text-slate-50 shadow-sm">
          ...
          <div className="space-y-2">
            ...
            <p className="text-sm leading-6 text-slate-300">
              Record the important movement, then mark the day complete when you have enough signal
              to trust the picture.
            </p>
          </div>
          <Button
            className="w-full bg-white text-slate-950 hover:bg-white/90"
            onClick={() => void onMarkTodayComplete()}
          >
            Mark today complete
          </Button>
        </div>
          ...
          <div className="flex flex-wrap items-start justify-between gap-4">
            <div>
              ...
              <h2 className="mt-3 font-display text-2xl text-foreground">
                Add today’s signal without friction.
              </h2>
              <p className="mt-2 text-sm leading-6 text-muted-foreground">
                Use this for the day’s money in, money out, or a debt payment.
                {activeEntryDate !== today
                  ? ` Catch-up date selected: ${formatShortDate(activeEntryDate)}.`
                  : ""}
              </p>
            </div>
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-lg font-semibold tabular-nums text-foreground">
                    {formatCurrency(summary.todayMoneyIn)}
                  </p>
                </div>
              </div>
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-lg font-semibold tabular-nums text-foreground">
                    {formatCurrency(summary.todayMoneyOut)}
                  </p>
                </div>
              </div>
            </div>
          ...
          <Card className="space-y-4">
            <div className="flex items-start justify-between gap-4">
              <div>
                ...
                <p className="mt-2 text-sm leading-6 text-muted-foreground">
                  Enough context to keep your footing without turning the app into a finance
                  dashboard.
                </p>
              </div>
            ...
            <div className="grid gap-3">
              <div className="flex items-center justify-between rounded-[22px] border border-border/70 bg-card/80 px-4 py-3">
                ...
                <strong className="tabular-nums text-foreground">
                  {formatCurrency(summary.monthMoneyIn)}
                </strong>
              </div>
              <div className="flex items-center justify-between rounded-[22px] border border-border/70 bg-card/80 px-4 py-3">
                <span className="text-sm text-muted-foreground">Money out</span>
                <strong className="tabular-nums text-foreground">
                  {formatCurrency(summary.monthMoneyOut)}
                </strong>
              </div>
              <div className="flex items-center justify-between rounded-[22px] border border-primary/10 bg-accent/70 px-4 py-3">
                ...
                <strong className="tabular-nums text-primary">
                  {summary.missedCheckInDaysCount}
                </strong>
              </div>
            </div>
          </Card>
          ...
          <Card className="space-y-4 border-warning/35 bg-warning/5">
            <div className="flex items-start justify-between gap-4">
              <div>
                ...
                <p className="mt-2 text-sm leading-6 text-muted-foreground">
                  Missed days are a signal, not a failure. You can mark a day partial and come back
                  later.
                </p>
              </div>
            ...
            {!showCatchUp ? (
              <div className="rounded-[22px] border border-border/70 bg-card/80 px-4 py-4 text-sm text-muted-foreground">
                Catch-up prompts are hidden in Settings. You can still backfill from Calendar or
                Ledger.
              </div>
            ) : missedDates.length === 0 ? (
              <div className="rounded-[22px] border border-border/70 bg-card/80 px-4 py-4 text-sm text-muted-foreground">
                No missed days right now.
              </div>
            ) : (
              <div className="space-y-3">
                    ...
                    <div className="flex items-start gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-muted-foreground">
                          Use quick add with this date or mark it partial now.
                        </p>
                      </div>
                    </div>
